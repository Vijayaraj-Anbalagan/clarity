...
export default function ChatInterface() {
  ...
  const sendMessageToAPI = async (query: string) => {
    ...
    try {
      const response = await axios.post('/api/langgraph', {
        query,
        history: chatHistory,
        sessionId,
      });
      ...
      // Append user's query and bot's response to the chat history
      setChatHistory([
        ...chatHistory,
     ...
     >
       ...
       <ScrollArea className="flex-grow">
         {chatSessions.map((query) => (
           ...
           >
             <div className="font-semibold text-white">{query.sessionName}</div>
